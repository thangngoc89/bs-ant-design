...
~props=
  makeTimePickerProps(
    ...
    ~placeholder?,
    ~prefixCls?,
    ~hideDisabledOptions=?Js.Option.map([@bs] (b => from_bool(b)), hideDisabledOptions),
    ~disabledHours?,
    ~disabledMinutes?,
    ~disabledSEconds?,
    ~getPopupContainer?,
    ...
    ~hourStep=?Js.Option.map([@bs] (b => from_bool(b)), hourStep),
    ~minuteStep=?Js.Option.map([@bs] (b => from_bool(b)), minuteStep),
    ~secondStep=?Js.Option.map([@bs] (b => from_bool(b)), secondStep),
